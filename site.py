...
def prepare_to_run():
    ...
    algo_container.add_algo(Algo(Path.path_w2v_multi_pos, "w2v multi pos", True))
    algo_container.add_algo(Algo(Path.path_FT_multi_pos, "fastText multi pos", True))
    algo_container.add_algo(Algo(Path.path_FT_nn_filtered, "fastText nn filtered"))
    algo_container.add_algo(Algo(Path.path_w2v_nn_filtered, "w2v nn filtered"))
    algo_container.add_algo(Algo(Path.path_odeds_algo_200, "Oded's algorithm 200 dim feature"))
    algo_container.add_algo(Algo(Path.path_odeds_algo, "Oded's algorithm"))
    algo_container.add_algo(Algo(Path.path_w2v_nn_pos_10, "w2v main (nn&pos) 10 feat", True, True))
    algo_container.add_algo(Algo(Path.path_w2v_nn_pos_100, "w2v main (nn&pos) 100 feat", True, True))
    algo_container.add_algo(Algo(Path.path_w2v_nn_pos_200, "w2v main (nn&pos) 200 feat", True, True))
    algo_container.add_algo(Algo(Path.path_w2v_twitter, "w2v twitter", False, True))
    algo_container.add_algo(Algo(Path.path_w2v_neg_20, "w2v nn&pos more negative(20)", True, True))
    algo_container.add_algo(
        Algo(Path.path_w2v_neg_20_min_20, "w2v nn&pos more negative(20) and higher min", True, True))
